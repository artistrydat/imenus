...
<section class="userHomeBanner img bg_loader" data-src="<?= base_url(!empty(restaurant($id)->cover_photo)?restaurant($id)->cover_photo:'assets/frontend/images/bg/bg_1.jpg') ;?>" style="background: url(<?= bg_loader();?>)"  >
  <div class="userHomebanner_wrapper">
  <div class="row">
    <div class="col-lg-5 col-sm-8 col-md-8">
      <div class="homeBanner_left">
        <h1><?= html_escape(!empty(restaurant($id)->name)?restaurant($id)->name:restaurant($id)->username); ?></h1>
        <p><?= !empty(restaurant($id)->about_short)?character_limiter(restaurant($id)->about_short,120):"";?></p>
  ...
  <div class="userHome_social">
    <ul>
      ...
      <li><a href="<?= redirect_url($social['whatsapp'],'whatsapp',$shop['dial_code']);?>"><i class="icofont-whatsapp whatsapp"></i></a></li>
      <?php endif; ?>
    </ul>
  </div>
</div>
