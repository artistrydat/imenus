					...
					<tbody id="sortable" class="sortable sorting">
						<?php $j=1; foreach ($value['items'] as $row): ?>
							   <tr style="background: #fff;" id='<?= $row['id']; ?>' >
							      ...
							      <td>
							         <?php if(isset($row['allergen']) && !empty($row['allergen'])): ?>
							         <label class="label bg-primary-soft"><?= !empty(lang('allergens'))?lang('allergens'):"allergens";?> : <?= html_escape($row['allergen'])?></label>
							         <?php endif;?>
