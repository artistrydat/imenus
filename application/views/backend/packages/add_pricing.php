					...
					<div class="row">
						<div class="col-md-6">
						    ...
						    <div class="form-group col-md-12">
						        ...
						        <input type="text" name="slug" id="slug" class="form-control" placeholder="<?= !empty(lang('slug'))?lang('slug'):"Slug";?>" value="<?= !empty($type_data['slug'])?html_escape($type_data['slug']):set_value('slug'); ?>">
						    ...
						    <div class="form-group col-md-6 ">
						       ...
						       <select name="order_limit" class="form-control" >
						       	 <option value=""><?= lang('select'); ?></option>
						       	 <?php foreach (order_limit() as $key => $order_limit): ?>
						       	 	<option value="<?= $key ;?>" <?= isset($type_data['order_limit']) && html_escape($type_data['order_limit'])==$key?'selected':'';?>><?= html_escape($order_limit) ;?></option>
						       	 <?php endforeach ?>
						       </select>
						    </div>
						     ...
						     <div class="form-group col-md-6 ">
						       ...
						       <select name="item_limit" class="form-control" >
						       	 <option value=""><?= lang('select'); ?></option>
						       	 <?php foreach (item_limit() as $key => $item_limit): ?>
						       	 	<option value="<?= $key ;?>" <?= isset($type_data['item_limit']) && html_escape($type_data['item_limit'])==$key?'selected':'';?>><?= html_escape($item_limit) ;?></option>
						       	 <?php endforeach ?>
						       </select>
						    </div>   
						</div> <!-- col-md-6 -->
							...
							<div class="form-group col-md-12">
						        ...
						        <div class="features">
						        	<?php $i=1; foreach($features as $row): ?>
						        		...
						        		<?php endif; ?>	
						        		<?php if(LICENSE == '6fa1b959a5580d843a4ea03422873009' && $row['slug'] == 'online-payment'): ?>
						        		<label class="custom-checkbox"><span><?= $i;?></span><input type="checkbox" <?= isset($feature_id['feature_id']) && $feature_id['feature_id']==$row['id']?"checked":'' ;?>  name="feature_id[]" value="<?= $row['id']?>"> &nbsp; &nbsp; <?= html_escape($row['features']); ?> <p class="label label-default fz-11">* Extended license</p></label>
						        	<?php else: ?>
