...
<?php foreach ($item_list as $key => $row): ?>
	<div class="col-md-6 q-sm">
		<div class="homeSingle_item singleCatItem">
			<div class="homeSingleImg menu-img img" style="background: url(<?= base_url($row['thumb']) ;?>);"></div>
			...
			<div class="homeItemDetails list_view ">
				<div class="homeItem_left">
					...
					<div class="homeItem_right singleCatItem">
					...
					<div class="price_section">
						<p>
							<?php if(!empty($row['allergen'])): ?>
								<span class="capital fz-13"><?= !empty(lang('allergens'))?lang('allergens'):'allregens' ;?>: <?= html_escape($row['allergen']) ;?></span>
							<?php endif;?>
						</p>
					...
					<div class="port_d_flex home_view">
						...
						<?php else: ?>
							...
							<?php if(shop($row['shop_id'])->stock_status == 1): ?>
								...
								<?php else: ?>
									<p class="fz-12 stock_counter ml-5"><span class="out_of_stock"><?= lang('out_of_stock') ;?></span></p>
								<?php endif;?>
